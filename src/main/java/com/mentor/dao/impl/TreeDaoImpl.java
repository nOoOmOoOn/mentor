...
public class TreeDaoImpl extends JdbcTemplateBaseDao implements TreeDao {
	...
	public List<Tree> getFathersByRank(Integer leftNum, Integer rightNum, Integer rankNum, Integer n) {
		SqlBuilder sqlBuilder=new SqlBuilder("select * from Tree where 1=1 and leftNum < ? and rightNum > ? and rankNum > ? ");
		sqlBuilder.setParam(leftNum);
		sqlBuilder.setParam(rightNum);
		sqlBuilder.setParam(rankNum-n-1);
		return query(sqlBuilder);
	}
	
	public List<Tree> getChildrenByRank(Integer leftNum, Integer rightNum, Integer rankNum, Integer n) {
		SqlBuilder sqlBuilder=new SqlBuilder("select * from Tree where 1=1 and leftNum > ? and rightNum < ? and rankNum = ? ");
		sqlBuilder.setParam(leftNum);
		sqlBuilder.setParam(rightNum);
		sqlBuilder.setParam(rankNum+n);
		return query(sqlBuilder);
	}
	
	public Integer getChildrenNumByRank(Integer leftNum, Integer rightNum, Integer rankNum, Integer n) {
		SqlBuilder sqlBuilder=new SqlBuilder("select count(*) from Tree where 1=1 and leftNum > ? and rightNum < ? and rankNum = ? ");
		sqlBuilder.setParam(leftNum);
		sqlBuilder.setParam(rightNum);
		sqlBuilder.setParam(rankNum+n);
		return queryForPrimitive(sqlBuilder, Integer.class);
